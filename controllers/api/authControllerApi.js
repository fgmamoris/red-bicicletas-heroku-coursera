...
//       ) {
//         const token = jwt.sign({ id: usuario.id }, req.app.get("secretKey"), {
//           expiresIn: "7d",
//         });
...
//       .save()
//       .then(() => {
//         const token = jwt.sign({ id: req.user.id }, req.app.get("secretKey"), {
//           expiresIn: "7d",
//         });
    ...
    Usuario.findOne({ email: req.body.email }, function (err, userInfo) {
      if (err) {
        ...
        if (
          ...
          //retorno el token generado par el usuario
          res.status(200).json({
            message: "Usuario encontrado!",
            data: {
              usuario: userInfo,
